...
class PaymentView(LoginRequiredMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            messages.error(self.request, 'Оплата не прошла')
            return redirect('products:home-page')
        except Exception:
            messages.error(self.request, 'Непривиденная ошибка')
            return redirect('products:home-page')
        ...
        messages.info(self.request, 'Платеж был успешно произведен')
    ...
    def post(self, *args, **kwargs):
            ...
            if form.is_valid():
                ...
                if order.promo_code_applied:
                    messages.warning(self.request, "Промокод успешно добавлен")
                    return redirect('checkout:checkout')
                try:
                    code = PromotionCode.objects.get(code=form.cleaned_data.get('code'))
                except PromotionCode.DoesNotExist:
                    messages.warning(self.request, "Промокод не найден")
                    return redirect('checkout:checkout')
